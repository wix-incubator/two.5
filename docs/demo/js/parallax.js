(function (factory) {
  ...
  image2.add(config.images[1], 'speed', 0, 1, 0.05).onFinishChange(restart);
  image2.add(config.images[1], 'filter', FILTER_CONF).onChange(filterChange(1));
  const image3 = gui.addFolder('image 3');
  image3.add(config.images[2], 'speed', 0, 1, 0.05).onFinishChange(restart);
  image3.add(config.images[2], 'filter', FILTER_CONF).onChange(filterChange(2));
  const image4 = gui.addFolder('Image 4');
  image4.add(config.images[3], 'speed', 0, 1, 0.05).onFinishChange(restart);
  image4.add(config.images[3], 'filter', FILTER_CONF).onChange(filterChange(3));
  const image5 = gui.addFolder('Image 5');
  image5.add(config.images[4], 'speed', 0, 1, 0.05).onFinishChange(restart);
  image5.add(config.images[4], 'filter', FILTER_CONF).onChange(filterChange(4));
  let instance;
